...
All notable changes to FH Switch will be documented in this file.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through FH Switch's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes FH Switch's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require FH Switch Local Routing to stay running.
...
- **FH Switch Repository Links Unified**: In-app, README, and release metadata now point at the public GitHub repository and releases instead of a standalone website.
...
- **Release Notes Template**: Surfaces the public GitHub repository in the release notes template.
- **Brand Surface**: Documented the GitHub repository and releases as the official distribution channels across READMEs and in-app references.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. FH Switch now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; FH Switch aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from FH Switch, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render FH Switch's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point FH Switch at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point FH Switch at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the FH Switch form.
...
- **Tray Menu ID Constant**: The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"fhswitch"`) across all call sites.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through FH Switch no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps FH Switch running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between FH Switch and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **Skill Auto-Backup**: Skill files are automatically backed up to `~/.fhswitch/skill-backups/` before uninstall, with metadata preserved in `meta.json`; old backups pruned to keep at most 20
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in FH Switch, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Crash Logging** - Panic hook captures crash info to `~/.fhswitch/crash.log` with full stack traces (#562)
...
- **Windows Title** - Display "FH Switch" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to FH Switch exported backups only; refresh providers immediately after import
...
- **ArchLinux installation support** (#259) - Added AUR installation via `paru -S fhswitch-bin`
...
- **Deep link support** - Import Gemini providers via `fhswitch://` protocol
...
#### Deep Link Protocol (fhswitch://)

- **Protocol registration** - `fhswitch://` URL scheme for one-click imports
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `FHSwitch-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `FHSwitch-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `FHSwitch-v{version}-Linux.AppImage` / `.deb`
...
#### Backup on v1→v2 Migration (fhswitch internal config)

- When the app detects an old v1 config structure at `~/.fhswitch/config.json`, it now creates a timestamped backup before writing the new v2 structure.
- Backup location: `~/.fhswitch/config.v1.backup.<timestamp>.json`
- This only concerns fhswitch's own metadata file; your actual provider files under `~/.claude/` and `~/.codex/` are untouched.
