...
- **アプリケーションレベルのウィンドウコントロール**: FH Switch が自前で最小化 / 最大化トグル / 閉じるボタンを描画するオプション設定。Linux Wayland での体験を大きく改善
...
FH Switch は Hermes Agent を Claude / Codex / Gemini / OpenCode / OpenClaw と並ぶ一等の管理対象アプリとして初めてサポートします。
...
- **ユーザー `providers:` dict の読み取り専用表示**: YAML に手書きされたプロバイダーエントリは FH Switch で読み取り専用カードとして表示され、深い設定は Hermes Web UI に委譲
...
- FH Switch が自前で最小化 / 最大化トグル / 閉じるボタンを描画するオプション設定を追加。システム装飾の代わりに使用 (#1119, 感謝 @git1677967754)
...
- FH Switch が参照する `openclaw.json` のカスタム位置を設定できるオプションを追加 (#1518, 感謝 @mrFranklin)

### Hermes 設定ディレクトリのカスタマイズ

- FH Switch が参照する `~/.hermes/config.yaml` のカスタム位置を設定できるオプションを追加。データ駆動 dispatch でサポート
...
- 深い YAML 設定は FH Switch フォームで重複させず、「Hermes Web UI を起動」ボタン経由で Web UI に直接委譲
...
- トレイ識別子をハードコーディング文字列 `"main"` から `TRAY_ID` 定数（`"fhswitch"`）に移行。すべての呼び出し箇所で同期 (#1978, 感謝 @lidaxian121)
...
- FH Switch 経由でラウンドトリップしても OAuth MCP `auth` ブロックが消失したり、無関係な YAML キーが汚染されたりしなくなった
...
これらの機能を有効にすることで、ユーザーは**すべての関連リスクを自己責任で受諾**したものとみなされます。FH Switch はこれらの機能の使用に起因するアカウントの制限、警告、サービス停止について一切の責任を負いません。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `FHSwitch-v3.14.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `FHSwitch-v3.14.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                     |
| -------------------------------- | -------------------------------------------------------- |
| `FHSwitch-v3.14.0-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ   |
| `FHSwitch-v3.14.0-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary       |
| `FHSwitch-v3.14.0-macOS.tar.gz` | Homebrew インストールと自動更新用                        |
