...
>
> FH Switch is a **fully free and open-source** desktop app, and we **do not charge users any fees**. Multiple imposter websites have recently been spotted impersonating FH Switch to solicit payments and harvest account credentials, with some users already reporting financial losses. Please only obtain the software through the official channels listed below:
>
> | Channel            | Only Official                                                                  |
> | ------------------ | ------------------------------------------------------------------------------ |
> | Source             | **[github.com/burnworld3d/fhswitch](https://github.com/burnworld3d/fhswitch)** |
> | Source             | **[github.com/burnworld3d/fhswitch](https://github.com/burnworld3d/fhswitch)** |
> | Downloads          | **[GitHub Releases](https://github.com/burnworld3d/fhswitch/releases)**        |
> | Author             | **[@burnworld3d](https://github.com/burnworld3d)**                               |
> | Report an Imposter | **[GitHub Issues](https://github.com/burnworld3d/fhswitch/issues)**            |
>
> **Any "FH Switch" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues so we can take down the imposter site as quickly as possible.
...
**[Use FH Switch to configure, manage, and switch Claude Desktop providers in one place](../user-manual/en/2-providers/2.6-claude-desktop.md)**
...
FH Switch now treats **Claude Desktop** as a first-class managed surface alongside Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes.

- New dedicated Claude Desktop panel that brokers third-party providers to Claude Desktop through FH Switch's in-app proxy gateway
...
ChatGPT Codex providers no longer rely on a hardcoded model selection — FH Switch fetches a **live model list** from the ChatGPT backend on demand.
...
Switching a Claude Desktop provider writes FH Switch's managed 3P profile and **requires restarting Claude Desktop** to take effect; proxy-mode providers require FH Switch's Local Routing to stay running while in use.
...
### FH Switch Repository Links Unified

In-app, README, and release metadata now point at the public GitHub repository and releases instead of a standalone website.
...
The release notes template now surfaces the public GitHub repository.

### Brand Surface

Documented the GitHub repository and releases as the official distribution channels across READMEs and in-app references.
...
**Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Routing Claude Desktop traffic through FH Switch's in-app proxy gateway to a third-party provider exposes those requests to that provider's billing, compliance, and data-retention policies — read the target provider's terms of service before using.

By enabling these features, users **accept all associated risks**. FH Switch is not responsible for any account restrictions, warnings, or service suspensions that result from using these features.

---

## Download & Installation

Visit [Releases](https://github.com/burnworld3d/fhswitch/releases/latest) to download the appropriate version.
...
| File                                     | Description                                           |
| ---------------------------------------- | ----------------------------------------------------- |
| `FHSwitch-v3.15.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `FHSwitch-v3.15.0-Windows-Portable.zip` | Portable, extract and run, no registry writes         |

### macOS

| File                             | Description                                             |
| -------------------------------- | ------------------------------------------------------- |
| `FHSwitch-v3.15.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications |
| `FHSwitch-v3.15.0-macOS.zip`    | Extract and drag into Applications, Universal Binary    |
| `FHSwitch-v3.15.0-macOS.tar.gz` | For Homebrew installation and auto-update               |

> macOS builds are Apple code-signed and notarized — install directly.

### Homebrew (macOS)

> 🎉 FH Switch is now available in the official Homebrew cask repository — no need to add a custom tap!
...
>
> - `FHSwitch-v3.15.0-Linux-x86_64.AppImage` / `.deb` / `.rpm`
> - `FHSwitch-v3.15.0-Linux-arm64.AppImage` / `.deb` / `.rpm`

| Distribution                            | Recommended | Installation                                                           |
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i FHSwitch-*.deb` or `sudo apt install ./FHSwitch-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i FHSwitch-*.rpm` or `sudo dnf install ./FHSwitch-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./FHSwitch-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | Add execute permission and run, or use AUR                             |
| Other distros / not sure                | `.AppImage` | `chmod +x FHSwitch-*.AppImage && ./FHSwitch-*.AppImage`              |
