...
FH Switch v3.8.0 是一次重大的架构升级版本，重构了数据持久化层和用户界面，为未来的云同步和本地代理功能奠定基础。
...
- **Windows**：`FHSwitch-v3.8.0-Windows.msi` 或 `-Portable.zip`
- **macOS**：`FHSwitch-v3.8.0-macOS.tar.gz` 或 `.zip`
- **Linux**：`FHSwitch-v3.8.0-Linux.AppImage` 或 `.deb`
...
- [@burnworld3d](https://github.com/burnworld3d) - BUG 修复和功能增强
- 社区成员的测试和反馈
