...
This section describes how to configure FH Switch according to your preferences.
...
FH Switch supports three languages:
...
When enabled, FH Switch automatically runs when the system starts.
...
Starting from v3.13.0, FH Switch adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger FH Switch via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If FH Switch runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, FH Switch automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that FH Switch uses when opening a terminal.
...
The storage location for FH Switch's own data, defaulting to `~/.fhswitch/`.
...
Configure FH Switch's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, FH Switch automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `fhswitch-sync`) |
...
- FH Switch automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, FH Switch displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current FH Switch version number, with support for:
