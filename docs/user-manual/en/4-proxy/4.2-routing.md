...
App routing means letting FH Switch route a specific application's API requests through the local routing service.
...
When routing is enabled, FH Switch modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, FH Switch will:
...
Before enabling routing, FH Switch backs up the original configuration and restores it when disabled.
