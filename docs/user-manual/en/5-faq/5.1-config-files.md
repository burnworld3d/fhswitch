...
## FH Switch Data Storage

### Storage Directory

Default location: `~/.fhswitch/`
...
```
~/.fhswitch/
├── fhswitch.db      # SQLite database (SSOT)
...
`fhswitch.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to FH Switch's "providers") |
...
FH Switch's priority when modifying configurations:

1. **FH Switch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by FH Switch)
- FH Switch's `settings.json`

### Not Recommended to Edit Manually

- `fhswitch.db` database file
- Backup files
...
FH Switch v3.7.0 migrated from JSON files to SQLite:
