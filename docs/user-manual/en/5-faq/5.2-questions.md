...
FH Switch for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/burnworld3d/fhswitch/releases).
...
2. Add FH Switch to your antivirus software's whitelist
...
chmod +x FHSwitch-*.AppImage

# If it still fails, try
./FHSwitch-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.fhswitch/` directory exists
2. Restore from backup: `~/.fhswitch/backups/`
...
**Solutions**:
1. Confirm the file was exported by FH Switch
...
**Windows**:
- Check taskbar settings to ensure the FH Switch icon is not hidden
...
2. Restart the app
3. Delete `~/.fhswitch/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask fhswitch`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and FH Switch runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
1. Visit [GitHub Issues](https://github.com/burnworld3d/fhswitch/issues)
...
- macOS/Linux: `~/.fhswitch/logs/`
- Windows: `%APPDATA%\fhswitch\logs\`
