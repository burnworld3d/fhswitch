...
```
~/.fhswitch/
├── fhswitch.db      # SQLite データベース（SSOT）
...
`fhswitch.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（FH Switch の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
FH Switch が設定を変更する際の優先順位：

1. **FH Switch データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（FH Switch がバックフィルする）
- FH Switch の `settings.json`

### 手動編集を推奨しないもの

- `fhswitch.db` データベースファイル
- バックアップファイル
...
FH Switch v3.7.0 で JSON ファイルから SQLite に移行しました：
