...
const FHSWITCH_SQL_EXPORT_HEADER: &str = "-- FH Switch SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_fhswitch_sql_export(sql_content)?;
    ...
    fn validate_fhswitch_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(FHSWITCH_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 FH Switch 导出的 SQL 备份文件。",
            "Only SQL backups exported by FH Switch are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("fhswitch.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("fhswitch-periodic-maintenance-backup-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("FHSWITCH_TEST_HOME", &test_home);
