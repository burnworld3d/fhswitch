...
pub enum SkillStorageLocation {
    /// FH Switch 管理目录 (~/.fhswitch/skills/)
    #[default]
    #[serde(rename = "fhswitch", alias = "fh_switch")]
    FhSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.fhswitch/skills/ 或 ~/.agents/skills/）
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::settings::get_skill_storage_location();
        let dir = match location {
            SkillStorageLocation::FhSwitch => get_app_config_dir().join("skills"),
    ...
    /// 获取 Skill 卸载备份目录（~/.fhswitch/skill-backups/）
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 FH Switch 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "fhswitch".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 FH Switch 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "fhswitch".to_string()));
